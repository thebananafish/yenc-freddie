...
void init_yenc(void);
static int encode_buffer(char *, char *, int, Crc32*, int *, int);
...
static char encode_string_doc[] = "encode_string(input_string, output_string, startcolumn, escapedots)";
		...
		)
{
	...
	while(encoded < bytes || bytes == 0){
		...
		encoded_bytes = encode_buffer(&read_buffer[0], &write_buffer[0], read_bytes, &crc, &col, 0);
		...
		)
{
	...
	static char *kwlist[] = { "string", "crc32", "column", "escapedots", NULL };
	...
	output_len = encode_buffer(input_buffer, output_buffer, input_len, &crc, &col, escapedots);
...
void init_yenc()
{
	Py_InitModule3("_yenc", funcs, "Raw yenc operations (mangled by Freddie)");
}
